var topology;
var nodes,childNodes, links, childLinks;
...
//         return d.expand ? "http://ww2.sinaimg.cn/large/412e82dbjw1dsbny7igx2j.jpg" : "http://ww4.sinaimg.cn/large/412e82dbjw1dsbnxezrrpj.jpg";
			 return d.type=='switch' ? "/switch.png" : "./host.png";
      })
      .attr("x", "-32px")
      .attr("y", "-32px")
      .attr("width", "48px")
...
function getTopology(){
	...
	getswnodes(function (swnodes) {
		...
		getswlink(function(swlinks){
		//	alert(JSON.stringify(swlinks))
			links=swlinks;
...
//从后台获取Switch之间的连接关系
function getswlink(callback){
		var swlinks=[];
		var xhr=getXMLHttpRequest();
		xhr.open("GET","/getswlinks",true);
		...
		if(xhr.readyState==4){
			if(xhr.status==200){				
					...
					{
						//alert(MyObject[i]["src-switch"]);
						var tmpjson = new Object()
						tmpjson.source=MyObject[i]["src-switch"];
						tmpjson.target=MyObject[i]["dst-switch"];						
						swlinks.push(tmpjson);
					}
					//alert(JSON.stringify(swlinks));
					//return swlinks;
					callback(swlinks);
		...
		if(xhr.readyState==4){
			if(xhr.status==200){				
					...
					for(var i=0;i<MyObject.length;i++)
					{
						//alert(MyObject[i]["src-switch"]);
						var tmpjson = new Object()
						tmpjson.id=MyObject[i].switchDPID;
						tmpjson.type="switch";
						tmpjson.status=1;				
						swnodes.push(tmpjson);
					}
					//alert(JSON.stringify(swnodes));
					callback(swnodes);
...
//从后台获取host信息，以及host和switch之间的链接关系
function gethostnodes(callback){
		var hostnodes=[];
		var hostlinks=[];
		var xhr=getXMLHttpRequest();
			...
			if(xhr.status==200){				
					...
					for(var i=0;i<MyObject.length;i++)
					{
						...
						if(MyObject[i].attachmentPoint.length!=0)
							dst=MyObject[i].attachmentPoint[0].switchDPID;
						if(MyObject[i].ipv4.length>1&&MyObject[i].ipv4[0]=="0.0.0.0")
							src=MyObject[i].ipv4[1];
						else 
							src=MyObject[i].ipv4;
						if(src=="0.0.0.0")
							continue;
						var tmphostnode=new Object;
						tmphostnode.id=src;
						tmphostnode.type='host';
						tmphostnode.status=1
						hostnodes.push(tmphostnode);
						var tmphostlink=new Object;
						tmphostlink.source=src;
						tmphostlink.target=dst;
						hostlinks.push(tmphostlink);
				

					}
					//alert(JSON.stringify(hostnodes));
					//alert(JSON.stringify(swnodes));
					callback(hostnodes,hostlinks);
