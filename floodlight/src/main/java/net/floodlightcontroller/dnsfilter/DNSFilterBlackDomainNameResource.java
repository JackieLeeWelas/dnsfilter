...
public class DNSFilterBlackDomainNameResource extends DNSFilterResourceBase{
	@Get("json")
    public Object handleGet() {
		return "{\"status\" : \"failure\", \"details\" : \"Use POST to operate BlackDomainName\"}";
		
	}
	//添加，删除域名黑名单，或查询域名是否在黑名单中
	@Post
    public String handlePost(String json) {
		...
		try {
			...
			case "add":
				if(dnsfilter.insertBlackDomain(domain))
					return "{\"status\" : \"success\", \"details\" : \""+domain+" has been inserted in blacklist\"}";
				else
					return "{\"status\" : \"failure\", \"details\" : \""+domain+" is not a correct domain\"}";
			case "remove":
				if(dnsfilter.removeBlackDomain(domain))
					return "{\"status\" : \"success\", \"details\" : \""+domain+" has been removed from blacklist\"}";
				else
					return "{\"status\" : \"failure\", \"details\" : \""+domain+" is not a correct domain or not in the blacklist\"}";
			case "search":
				if(dnsfilter.hasBlackDomain(domain))
					return "{\"status\" : \"success\", \"details\" : \""+domain+" in the blacklist\"}";
				else
					return "{\"status\" : \"failure\", \"details\" : \""+domain+" is not a correct domain or not in the blacklist\"}";
			default:
