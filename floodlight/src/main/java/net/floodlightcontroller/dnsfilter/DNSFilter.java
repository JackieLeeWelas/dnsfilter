...
public class DNSFilter implements IFloodlightModule, IOFMessageListener,
		IDNSFilterService {
	...
	public net.floodlightcontroller.core.IListener.Command receive(
		...
		if (!this.SwFilterSet.contains(sw.getId())
				&& !this.isFilterHostIP(cntx) && !this.isFilterHostMAC(cntx))
		...
		if (eth.getPayload() instanceof IPv4) {
			...
			if (ip_pkt.getPayload() instanceof UDP) {
				...
				if (dst_p.equals(TransportPort.of(53))) {
					...
					String src2dst = " from " + src.toString() + ":"
							+ src_p.toString() + " to " + dst.toString() + ":"
							+ dst_p.toString() + " seen on switch: "
							+ sw.getId();
					logger.info("DNS packet  {},queryname:{} ", src2dst,
							dns_pkt.toString());
					logger.info("blacklist has:{}",
							blacklist.has(dns_pkt.toString()));
					logger.info("whitelist has:{}",
							whitelist.has(dns_pkt.toString()));

					// 不检查去往DNS 重定向服务的数据包
					if (dst.equals(this.DNSRedirectIp)
							|| src.equals(this.DNSRedirectIp))
						return Command.CONTINUE;

					// 检查该DNS请求的域名是否在黑名单中，
					if (blacklist.has(dns_pkt.toString())) {
						// 记录该数据包
						this.record(sw, src, dst, dns_pkt.toString());
	...
	// 记录解析恶意域名的DNS数据包
	protected void record(IOFSwitch sw, IPv4Address src, IPv4Address dst,
			String query_name) {
		...
		try {
			...
			String out = df.format(now) + " " + src.toString() + " "
					+ dst.toString() + " " + sw.getId() + " " + query_name
					+ "\n";
	...
	public void init(FloodlightModuleContext context)
		...
		messageDamper = new OFMessageDamper(1000, EnumSet.of(OFType.FLOW_MOD),
				250);
	...
	public boolean cheackDomainName(String name) {
		...
		for (int i = 0; i < name.length(); i++) {
			if (!(Character.isDigit(letters[i])
					|| Character.isLowerCase(letters[i]) || letters[i] == '.' || letters[i] == '-')) {
				return false;
			}
	...
	protected void redirect(IOFSwitch sw, OFPacketIn pi, IPv4Address src,
			IPv4Address dst, TransportPort src_p, TransportPort dst_p) {

		List<OFAction> actionsRequest = new ArrayList<OFAction>();
		List<OFAction> actionsReply = new ArrayList<OFAction>();

		actionsRequest.add(sw.getOFFactory().actions()
				.setNwDst(this.DNSRedirectIp));
		actionsRequest.add(sw.getOFFactory().actions()
				.output(OFPort.FLOOD, Integer.MAX_VALUE));
		...
		actionsReply.add(sw.getOFFactory().actions()
				.output(OFPort.FLOOD, Integer.MAX_VALUE));
		// 修改DNS响应包的源地址。
		this.AddFlow2DNS(sw, this.DNSRedirectIp, src, dst_p, src_p,
				actionsReply);

		// 将当前报文发送回交换机
		List<OFAction> actions = new ArrayList<OFAction>();
		actions.add(sw.getOFFactory().actions()
				.output(OFPort.TABLE, Integer.MAX_VALUE));
	...
	protected void AddFlow2DNS(IOFSwitch sw, IPv4Address src, IPv4Address dst,
		...
		fmb.setCookie(cookie).setHardTimeout(0).setIdleTimeout(5)
				.setBufferId(OFBufferId.NO_BUFFER).setMatch(mb.build())
				.setActions(actions).setPriority(1000);

		try {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"write DNS redirect flow-mod sw={} match={} flow-mod={}",
	...
	void SendPacketBackSwitch(IOFSwitch sw, OFPacketIn pi,
	...
	public void setProxy(DatapathId switchid, IPv4Address src_ip,
			...
			MacAddress dst_mac, MacAddress proxy_mac, int hardtimeout,
			int idletimeout, OFPort monitor_port) {

		IOFSwitch sw = switchService.getSwitch(switchid);

		this.addProxyFlow(sw, this.setProxyMatch(sw, src_ip, proxy_ip), this
				.setProxyAction(sw, proxy_ip, dst_ip, proxy_mac, dst_mac,
						monitor_port), hardtimeout, idletimeout);

		this.addProxyFlow(sw, this.setProxyMatch(sw, dst_ip, proxy_ip), this
				.setProxyAction(sw, proxy_ip, src_ip, proxy_mac, src_mac,
						monitor_port), hardtimeout, idletimeout);
	...
	protected List<OFAction> setProxyAction(IOFSwitch sw, IPv4Address src_ip,
			IPv4Address dst_ip, MacAddress src_mac, MacAddress dst_mac,
			OFPort monitor_port) {
		...
		actions.add(sw.getOFFactory().actions()
				.output(monitor_port, Integer.MAX_VALUE));
		actions.add(sw.getOFFactory().actions()
				.output(OFPort.IN_PORT, Integer.MAX_VALUE));
		return actions;
	}

	protected void addProxyFlow(IOFSwitch sw, Match match,
			List<OFAction> actions, int hardtimeout, int idletimeout) {

		OFFlowMod.Builder fmb = sw.getOFFactory().buildFlowAdd();
		U64 cookie = AppCookie.makeCookie(2, 0);

		fmb.setCookie(cookie).setHardTimeout(hardtimeout)
				.setIdleTimeout(idletimeout).setBufferId(OFBufferId.NO_BUFFER)
				.setMatch(match).setActions(actions).setPriority(1000);

		try {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"write proxy  flow-mod sw={} match={} flow-mod={}",
